...
# 検証リストのパス（修正）
validation_list_path = "/home/nagashimadaichi/dev/melo-tts/melo/data/vtuber/val.list"
output_path = "/home/nagashimadaichi/dev/melo-tts/melo/data/vtuber/val.list.fixed"
...
with open(validation_list_path, 'r') as f_in, open(output_path, 'w') as f_out:
    for line in f_in:
        count_total += 1
        parts = line.split('|')
        if len(parts) < 3:
...
# 新しいリストで元のリストを置き換え
os.rename(output_path, validation_list_path)
